...
# GAOptimizerのためのグローバル変数
$Global:GAOptimizer = $null
...
$Global:Processes = @(
    ...
    @{
        ...
        Action = {
            param($data)
            # data: {Places: array}
            if ($Global:GAOptimizer) {
                return @{ status = "already running" }
            }
            $Global:GAOptimizer = [GAOptimizer]::new()
            $Global:GAOptimizer.Start($data)
    ...
    @{
        ...
        Action = {
            ...
            return $Global:GAOptimizer.Status()
        }
    },
    @{
        Name   = "getBest"
        Action = {
            ...
            return $Global:GAOptimizer.GetBest()
        }
    },
    @{
        Name   = "stopGA"
        Action = {
            ...
            $Global:GAOptimizer.Stop()
            $finalStatus = $Global:GAOptimizer.Status()
            $Global:GAOptimizer = $null
            return @{ status = "stopped"; final = $finalStatus }
        }
    }
