...
export default class UIManager {
  ...
  initUIHandlers() {
    document
      .getElementById(this.selector.controls.toggleLockBtnId)
      .addEventListener("click", () =>
        this.selector.imageHandler.toggleLockMode()
      );
  ...
  initGpxLoadButton() {
    ...
    input.addEventListener("change", (e) => {
      ...
      reader.onload = (ev) => {
        ...
        // ✅ 正式 Model には addPoint 経由で追加（2重追加を防ぐ）
        newPts.forEach((p) => {
          this.selector.markerHandler.addPoint(p);
          const newMarker = this.selector.markerHandler.markers.at(-1);
          this.selector.fetchAddressAsync(p, newMarker);
        });
  ...
  updateListUI() {
    ...
    const selected = this.selector.markerHandler.selectedIndex;
  ...
  finish() {
    ...
    fetch("/choice", {
      ...
      body: JSON.stringify(this.selector.points),
    })
