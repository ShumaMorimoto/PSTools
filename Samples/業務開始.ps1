...
$citems | ForEach-Object { $body += $_.Start.toString("M/d(ddd) hh:mm-") + $_.End.toString("hh:mm ") + $_.Subject + " @" + $_.Location + "`r`n" }
...
$select | ForEach-Object { $item = $o.GetApo($_.EntryID)
    $item.Body += "Zoom"
...
$citem = $citems | Select-Object $header 
