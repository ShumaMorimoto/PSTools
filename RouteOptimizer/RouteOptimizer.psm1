...
"@
    ...
    static [GPXDocument] FromMap([string]$Keyword) {
        return [GPXDocumentFactory]; ; FromMap($Keyword, $false)
    }
    ...
    static [GPXDocument] FromMapEdit() {
        return [GPXDocumentFactory]::FromMapEdit($null)
    }
    static [GPXDocument] FromMapEdit([GPXDocument]$gpx) {
        if (-not $gpx) { 
            $gpx = [GPXDocument]::new(@{
                    name = "MapEdit"
                    desc = ""
        ...
        # 1. 元の trkpt を取得
        $origTrkpts = $gpx.GetTrkPts()

        # 2. Choice‑Places を呼び出す（UI 起動）
        $editedTrkpts = Choice-Places -Places $origTrkpts

        # 3. 編集されなかった場合はそのまま返す
        if (-not $editedTrkpts) { return $gpx }

        # 4. trkpt[] を GPXDocument に反映（SetTrkPts が全部やる）
        $gpx.SetTrkPts($editedTrkpts)
