...
export default class MarkerHandler {
    ...
    addPoint(info) {
        ...
        const icon = L.ExtraMarkers.icon({
            icon: "fa-number", number: this.pointList.length,
            markerColor: "blue", shape: "circle"
        });

        const m = L.marker([info.lat, info.lon], { draggable: true, icon: icon }).addTo(this.selector.map);
        this.markers.push(m);
    ...
    clearMarkers() {
        this.markers.forEach(m => { if (m) this.selector.map.removeLayer(m); });
        this.markers = [];
        this.pointList = [];
        this.selectedIndex = null;
        this.selector.uiManager.updateListUI();
